...
originaux. ED5A fournit une factorisation bipartite finie typée en coordonnées,
ED5B des tranches bipartites et profils de normes purement coordonnés, et ED5C
des masques finis de coordonnées bipartites transportés dans l'espace ambiant.
...
displacement between transported and original projectors. ED5A provides a
typed finite bipartite coordinate factorization, ED5B purely coordinate
bipartite slices and norm profiles, and ED5C finite bipartite coordinate masks
transported into the ambient space. No stable local API or decoherence theorem
is announced.
-/
