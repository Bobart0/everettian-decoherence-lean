...
défaut statewise, ED3B son uniformisation par norme d'opérateur, et ED4A la
composition de deux transformations à perspective fixée. Aucune API locale
...
its operator-norm uniformization, and ED4A the composition of two
transformations at a fixed perspective. No stable local API or decoherence
