...
There is an equivalent formulation in terms of a pair of projective
measurements.  Put
\[
  Q_i:=UP_iU^*,
  \qquad
  Q_S:=\sum_{i\in S}Q_i=UP_SU^*.
\]
Then
\begin{equation}\label{eq:pvm-distance-identities}
  \|[P_i,U]\|_{\op}=\|P_i-Q_i\|_{\op},
  \qquad
  C_S=\|P_S-Q_S\|_{\op}.
\end{equation}
Indeed, right multiplication by $U^*$ gives
$[P_i,U]U^*=P_i-UP_iU^*$, and similarly for $P_S$.
For two POVMs with the same outcome set, the operational distance is
\[
  D_{\mathrm{op}}(P,Q)
  :=
  \max_{S\subseteq I}
  \left\|\sum_{i\in S}(P_i-Q_i)\right\|_{\op};
\]
this is the standard worst-case total-variation distance between their outcome
statistics \cite{NavascuesPopescu2014,MaciejewskiZimborasOszmaniec2020}.
Thus
\begin{equation}\label{eq:cmax-operational}
  \cmax=D_{\mathrm{op}}(P,Q).
\end{equation}
Conversely, if $(P_i)$ and $(Q_i)$ are finite projective decompositions with
$\operatorname{rank}P_i=\operatorname{rank}Q_i$ for every $i$, a unitary can
be chosen that maps each $P_iH$ onto $Q_iH$.  Hence all results below admit an
equivalent formulation for pairs of projective measurements with matching
rank profiles.  This interpretation is optional: the proofs use only
projection and operator-norm geometry.
...
\begin{corollary}[Operational-distance form]\label{cor:operational-distance}
Let $P=(P_i)_{i\in I}$ and $Q=(Q_i)_{i\in I}$ be finite projective
decompositions with matching rank profiles.  Then
\[
  \boxed{
  D_{\mathrm{op}}(P,Q)^2
  \le
  \frac12\sum_{i\in I}\|P_i-Q_i\|_{\op}^2.
  }
\]
...
\begin{proof}
Choose a unitary $U$ with $Q_i=UP_iU^*$ for all $i$ and use
\eqref{eq:pvm-distance-identities}, \eqref{eq:cmax-operational}, and
...
\[
  ...
  \qquad
  \zeta_c:=
    \frac{P_c z_c}{\sqrt{r_c}}+\sqrt{p_c}\,v.
\]
Thus $\zeta_c$ is obtained from $\xi_c$ by replacing its normalized residual
component by $\sqrt{p_c}\,v$.  The witness geometry gives three facts:
\begin{align}
...
\end{equation}
Moreover the cell-$1$ budget is no larger than either the cell-$0$ or
cell-$2$ budget.  Hence cells $0$ and $2$ are a top-two pair, and omitting
cell $1$ gives the optimal two-cell tail
...
The cut functional also has a standard interpretation in quantum
measurement theory.  The operational distance between two finite POVMs is
the maximum, over subsets of outcomes, of the operator norm of the
corresponding summed effect difference
\cite{NavascuesPopescu2014,MaciejewskiZimborasOszmaniec2020}.  It is the
largest total-variation distance between the two outcome distributions over
input states, and it is closely connected with measurement discrimination
\cite{PuchalaPawelaKrawiecKukulski2018}.  Equations
\eqref{eq:pvm-distance-identities}--\eqref{eq:cmax-operational} show that the
finite cut envelope in this paper is exactly this operational distance for a
projective measurement and its unitary conjugate.  A generic triangle
estimate gives
\[
  D_{\mathrm{op}}(P,Q)
  \le \frac12\sum_i\|P_i-Q_i\|_{\op},
\]
whereas Corollary~\ref{cor:operational-distance} uses projectivity and the
matched-rank structure to obtain the sharp dimension-free quadratic bound.
The equality classification and near-equality stability developed here are
statements about when that quadratic bound is saturated or nearly saturated.
...
\cite{Li2004}.  More recently, Conde derived norm inequalities and extremal
characterizations for commutators and anticommutators associated with two
orthogonal projections \cite{Conde2022}.  Those works provide context for
individual projection pairs, while
