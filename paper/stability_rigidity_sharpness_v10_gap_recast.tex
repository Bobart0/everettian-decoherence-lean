...
For example, in $\C^2$ take the coordinate rank-one decomposition
$(P_1,P_2)$ and let $(Q_1,Q_2)$ be obtained from it by a nontrivial planar
rotation through an angle $\theta$.  Then
\[
  p_1=p_2=\sin^2\theta,\qquad
  \dd^2=2\sin^2\theta,\qquad
  \gmax^2=\sin^2\theta,
\]
so equality holds.
...
$\sum_i\|P_i-Q_i\|_{\op}^2$.  Pinching methods provide another nearby
operator-theoretic language for isolating diagonal and off-diagonal blocks;
Bhatia and Kittaneh, for example, relate commutators, pinchings, and spectral
variation under unitarily invariant norms \cite{BhatiaKittaneh2008Pinching}.
The envelope is elementary once the two-sided cut estimate has been isolated;
the nontrivial part of the present work is the inverse theory of its equality
and near-equality regimes.
