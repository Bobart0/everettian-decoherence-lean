...
\[
  ...
  \right]=4.
\]
The complete argument is machine-checked in Lean 4.  It forms the downstream
layer of an end-to-end finite-dimensional quantum-foundations library whose
audited upstream developments also formalize Busch's and Gleason's
...
The present repository is the downstream layer of a pinned four-repository
formalization stack,
\begin{center}
\small
\texttt{gleason-theorem-lean}
$\to$
\texttt{quantum-foundations-lean}
$\to$\\
\texttt{everettian-probability-lean}
$\to$
\texttt{everettian-decoherence-lean}
\end{center}
The stack includes machine-checked finite-dimensional versions of the Busch
and Gleason representation theorems, Naimark dilation, Wigner's theorem,
Uhlhorn-type uniqueness, Riedel's branch-decomposition theorem, and Kent's
contrary-inference construction, together with conditional Born-probability
and exact-finite layers.  These named results provide the formalization
context and search-visible provenance of the present work; they are not
premises of the cut-envelope stability theorem unless explicitly stated.

% TODO before submission:
% 1. Replace repository commit by archival release/DOI.
% 2. Adapt notation/template to target journal.
...
\]
This repository is downstream of the pinned chain
\[
  \texttt{gleason-theorem-lean}
  \to
  \texttt{quantum-foundations-lean}
  ...
  \texttt{everettian-decoherence-lean}.
\]
Among the audited public results in the upstream stack are
\texttt{Gleason.busch}, \texttt{Gleason.gleason},
\texttt{QuantumFoundations.naimark},
\texttt{QuantumFoundations.Wigner.wigner},
\texttt{QuantumFoundations.Uhlhorn.uhlhorn\_finite\_dim},
\texttt{QuantumFoundations.BranchesRiedel.riedel}, and
\texttt{QuantumFoundations.HistoriesKent.contrary\_inferences}.
The Kent construction is included as a machine-checked conceptual contrast in
the upstream foundations library, not as a premise of the theorem proved in
this article.
The Lean development works with canonical coordinate Hilbert spaces
$H_n\cong\C^n$.  Since all statements in this paper are invariant under
unitary change of coordinates, the coordinate-free finite-dimensional
formulation is mathematically equivalent; the repository itself checks the
canonical-coordinate version.
...
For manuscript version 0.8, the reference development is the CI-verified
...
Optimal top-two tail &
\texttt{stabilitySharpnessTopTwoTail\_eq\_Tau}\\
Sharpness ratio &
