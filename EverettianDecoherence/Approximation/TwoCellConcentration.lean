import EverettianDecoherence.Approximation.CutConcentration
import EverettianDecoherence.Approximation.SubsetCommutatorOpNormBounds

/-!
**FR.** Concentration bilatérale sur un cut et son complément. Si
E = delta^2 - 2 c^2 est petit relativement à c^2, alors une cellule de S et
une cellule de S^c capturent presque tout le budget quadratique global.

**EN.** Bilateral concentration on a cut and its complement. If
E = delta^2 - 2 c^2 is small relative to c^2, then one cell of S and one cell
of S^c capture almost all of the global quadratic budget.
-/
...
/-- Two-cell concentration relative to an arbitrary cut. Here
C is the aggregate commutator norm, A is delta squared, and E = A - 2 C^2. -/
theorem exists_two_cell_concentration_of_cut
    {n : ℕ} (D : Perspective n)
    (U : H n ≃ₗᵢ[ℂ] H n)
    ...
    ∃ i ∈ S, ∃ j ∈ Sᶜ,
      operatorNormProjectorCommutatorL2 D U ^ 2 -
        ...
        2 * (operatorNormProjectorCommutatorL2 D U ^ 2 -
          2 * ‖recordSubsetProjectorCommutatorCLM D U S‖ ^ 2) +
        18 * (operatorNormProjectorCommutatorL2 D U ^ 2 -
          2 * ‖recordSubsetProjectorCommutatorCLM D U S‖ ^ 2) /
  ...
  obtain ⟨i, hiS, hi⟩ :=
    exists_cell_concentration_of_cut D U S hcpos
  ...
  obtain ⟨j, hjT, hj⟩ :=
    exists_cell_concentration_of_cut D U Sᶜ hcposT hTcut
  have hi' :
      AS - incomingWitnessP D U i ≤
        2 * eS + 18 * eS / (C ^ 2 - eS) := by
    simpa [AS, C, eS] using hi
  have hj' :
      AT - incomingWitnessP D U j ≤
        2 * eT + 18 * eT / (C ^ 2 - eT) := by
    simpa [AT, C, eT,
  ...
  have hsum :
    ...
    calc
      (AS - incomingWitnessP D U i) +
        ...
        add_le_add hi' hj'
      _ = 2 * (eS + eT) +
          18 * (eS / (C ^ 2 - eS) + eT / (C ^ 2 - eT)) := by ring
      _ ≤ 2 * E + 18 * (E / d) := by
        rw [hsumE]
        gcongr
      _ = 2 * E + 18 * E / d := by ring
